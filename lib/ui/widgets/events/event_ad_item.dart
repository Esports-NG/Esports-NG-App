...
class EventAdItem extends StatefulWidget {
  final EventModel item;
  final bool? onDetailsPage;
  const EventAdItem({super.key, required this.item, this.onDetailsPage});

  @override
  State<EventAdItem> createState() => _EventAdItemState();
}

class _EventAdItemState extends State<EventAdItem> {
  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        if (widget.item.type == "tournament") {
          Get.to(() => AccountTournamentDetail(item: widget.item));
        } else {
          Get.to(() => SocialEventDetails(item: widget.item));
        }
      },
      child: Container(
        width: Get.width * 0.9,
        ...
        child: Column(
          ...
          children: [
            GestureDetector(
              onTap: () =>
                  Helpers().showImagePopup(context, "${widget.item.banner}"),
              child: Stack(
                children: [
                  ...
                  Visibility(
                    ...
                    child: Column(
                      children: [
                        ...
                        textItem(
                          title: 'Game Modes: ',
                          subTitle: widget.item.gameMode.toString(),
                        ),
                      ],
                    ),
                  ),
