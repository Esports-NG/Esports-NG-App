...
class _PostItemForProfileState extends State<PostItemForProfile> {
  ...
  Future<bool> onLikeButtonTapped(bool isLiked) async {
    if (postController.postStatus != PostStatus.loading) {
      postController.likePost(widget.item.slug!);
    }
    return !isLiked;
  }
                          ...
                          children: [
                            (widget.item.author!.profile!.profilePicture ==
                                ...
                                : GestureDetector(
                                    onTap: () {
                                      Get.to(() => UserDetails(
                                          slug: widget.item.author!.userName!));
                                    },
                                    child: CachedNetworkImage(
                                      height: Get.height * 0.035,
                                      width: Get.height * 0.035,
                          ...
                          children: [
                            (widget.item.repost!.author!.profile!
                                ...
                                : GestureDetector(
                                    onTap: () => Get.to(() => UserDetails(
                                        slug:
                                            widget.item.repost!.author!.slug!)),
                                    child: CachedNetworkImage(
                                      height: Get.height * 0.035,
                                      width: Get.height * 0.035,
                                      placeholder: (context, url) =>
  ...
  void postMenu() async {
    ...
    if (selectedMenuItem == '0' &&
      ...
      await authController.followUser(widget.item.author!.slug!);
